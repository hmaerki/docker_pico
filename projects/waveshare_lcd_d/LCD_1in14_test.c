...
int main(void)
{
    ...
    const char INPUTLEN = 30;
    char input_buffer[INPUTLEN];
    int input_idx = 0;
        ...
        if (c != PICO_ERROR_TIMEOUT)
        {
            if ((c == '\n') || (c == '\r'))
            {
                if (input_idx > 0)
                {
                    ...
                    Paint_Clear(BLACK);
                    Paint_DrawString_EN(20, 20, input_buffer, &Font20, WHITE, BLACK);
                    Paint_DrawString_EN(20, 60, input_buffer, &Font24, GREEN, BLACK);
                }
            }
            else
            {
